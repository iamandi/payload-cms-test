...
const Blocks = (props) => {
    const { label, name, path: pathFromProps, blocks, labels = {
        singular: 'Block',
        plural: 'Blocks',
    }, fieldTypes, maxRows, minRows, required, validate = blocksValidator, permissions, admin: { readOnly, }, } = props;
    ...
    return (React.createElement(RenderBlocks, { onDragEnd: onDragEnd, label: label, showError: showError, errorMessage: errorMessage, rows: rows, labels: labels, addRow: addRow, removeRow: removeRow, moveRow: moveRow, path: path, name: name, fieldTypes: fieldTypes, toggleCollapse: toggleCollapse, permissions: permissions, value: value, blocks: blocks, readOnly: readOnly, minRows: minRows, maxRows: maxRows, required: required }));
};
const RenderBlocks = React.memo((props) => {
    const { onDragEnd, label, showError, errorMessage, rows, labels, addRow, removeRow, moveRow, path, fieldTypes, permissions, value, toggleCollapse, blocks, readOnly, minRows, maxRows, required, } = props;
    ...
    return (React.createElement(DragDropContext, { onDragEnd: onDragEnd },
        React.createElement("div", { className: baseClass },
            ...
            React.createElement(Droppable, { droppableId: "blocks-drop", isDropDisabled: readOnly }, (provided) => (React.createElement("div", Object.assign({ ref: provided.innerRef }, provided.droppableProps),
                rows.length > 0 && rows.map((row, i) => {
                    const { blockType } = row;
                    const blockToRender = blocks.find((block) => block.slug === blockType);
                    if (blockToRender) {
                        return (React.createElement(DraggableSection, { readOnly: readOnly, key: row.key, id: row.key, blockType: "blocks", blocks: blocks, label: blockToRender?.labels?.singular, isOpen: row.open, rowCount: rows.length, rowIndex: i, addRow: addRow, removeRow: removeRow, moveRow: moveRow, toggleRowCollapse: toggleCollapse, parentPath: path, fieldTypes: fieldTypes, permissions: permissions, hasMaxRows: hasMaxRows, fieldSchema: [
